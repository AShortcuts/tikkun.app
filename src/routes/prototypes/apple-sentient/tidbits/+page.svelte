...
<svelte:head>
  <title>Tidbits | Apple/Sentient prototype</title>
  ...
  {#if tidbits.length === 0}
    <section class="site-section site-empty-section" aria-labelledby="tidbits-empty-title">
      <div class="site-empty-state">
        ...
        <a class="site-inline-action" href={resolve('/prototypes/apple-sentient/readings/')}>View available readings</a>
      </div>
    </section>
  {:else}
      ...
      <div class="tidbit-list" role="list">
        {#each tidbits as tidbit (tidbit.slug)}
          <article class="tidbit-card" role="listitem">
            ...
            <h2>{tidbit.title}</h2>
            <p>{tidbit.summary}</p>
            <a href={resolve(`/prototypes/apple-sentient/tidbits/${tidbit.slug}/`)}>Read note</a>
          </article>
        {/each}
      </div>
    </section>
