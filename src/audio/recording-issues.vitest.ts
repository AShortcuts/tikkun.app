...
test('drops stored issues from incompatible tokenization versions', () => {
  ...
  expect(storage.getItem('tikkun.recording-issues.v1:bereshit-1')).toBeNull()
  expect(
    storage.getItem('tikkun.recording-issues.v1:bereshit-1:quarantine')
  ).not.toBeNull()
})
...
test('salvages valid issues while quarantining a malformed local payload', () => {
  ...
  expect(loadRecordingIssues(storage, 'bereshit-1', 'v2')).toEqual([validIssue])
  expect(loadRecordingIssues(storage, 'bereshit-1', 'v2')).toEqual([validIssue])
  expect(
    JSON.parse(
      storage.getItem('tikkun.recording-issues.v1:bereshit-1') ?? '[]'
    )
  ).toEqual([validIssue])
  expect(
    storage.getItem('tikkun.recording-issues.v1:bereshit-1:quarantine')
...
test.each([
  ...
  expect(filterRecordingIssues([issue], 'bereshit-1', 'v2')).toEqual([])
  expect(parseRecordingIssues([issue], 'bereshit-1', 'v2')).toBeNull()
})
...
test('rejects duplicate issue identities in an atomic published issue list', () => {
  ...
  expect(parseRecordingIssues([issue, issue], 'bereshit-1', 'v2')).toBeNull()
})
...
test('handles denied issue reads and surfaces denied writes', () => {
  ...
  expect(() => saveRecordingIssues(writeStorage, 'bereshit-1', [])).toThrow(
    RecordingIssueStorageError
  )
})
