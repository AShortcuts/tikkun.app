...
export function aliyahMembershipsForFocalLine({
  runs,
  lineRun,
  focalRef,
  aliyot,
  ...
  const identitiesFor = (candidates: LeiningAliyah[]) =>
    lineRun
      ? candidates.flatMap((aliyah) =>
          aliyah.index ? [aliyahIdentity(lineRun, aliyah.index)] : []
        )
      : []

  const starts = identitiesFor(aliyahStarts)
  if (starts.length) return starts
  if (focalRef) return aliyahMembershipsAtRef(runs, focalRef)
  return identitiesFor(aliyot)
}
