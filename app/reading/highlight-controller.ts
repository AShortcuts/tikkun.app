...
const HIGHLIGHT_SCROLL_ROUNDING_EPSILON_PX = 0.5
...
export class HighlightController {
  ...
  async activateCue(cue: WordCue, options?: HighlightActivationOptions) {
  ...
  async activateTokenKey(
    tokenKey: string,
    { scroll = true, scrollBehavior = 'smooth' }: HighlightActivationOptions = {}
  ) {
    const generation = ++this.activationGeneration
    return this.applyTokenKey(tokenKey, { scroll, scrollBehavior }, generation)
  }
  ...
  private applyTokenKey(
    ...
    if (target && scroll) {
      this.scrollTokenIntoView(target, scrollBehavior)
    }
    return target
  }

  step(delta: -1 | 1, options?: HighlightActivationOptions) {
  ...
  private scrollTokenIntoView(element: HTMLElement, behavior: ScrollBehavior) {
    centerElementInScrollRoot(this.book, element, {
      behavior,
      minimumScrollDistance: HIGHLIGHT_SCROLL_ROUNDING_EPSILON_PX,
    })
  }
