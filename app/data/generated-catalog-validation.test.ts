...
import { aliyahVideos } from '../../generated/video-manifest.ts'
...
const pagePayloads = import.meta.glob<unknown>('../../text/pages/**/*.json', {
  eager: true,
  import: 'default',
})
const tocPayloads = import.meta.glob<unknown>('../../text/*-toc.json', {
  eager: true,
  import: 'default',
})
...
test('generated audio and video catalogs have unique, internally consistent identities', async () => {
  ...
  for (const recording of audioRecordings) {
    ...
    if (recording.status === 'available' && recording.playSrc.startsWith('/')) {
      ...
      const mediaPath = path.resolve(repoRoot, 'site', sitePath)
      expect(mediaPath.startsWith(path.resolve(repoRoot, 'site') + path.sep)).toBe(true)
  ...
  for (const [parshaSlug, recordings] of Object.entries(audioRecordingsByParsha)) {
    expect(canonicalizeParshaSlug(parshaSlug) ?? parshaSlug).toBe(parshaSlug)
...
test('TOC locations reference existing Page Data and Torah mappings are bidirectional', () => {
  ...
  for (const [tocPath, toc] of Object.entries(tocPayloads)) {
    const scroll = /\/([^/]+)-toc\.json$/.exec(tocPath)?.[1]
