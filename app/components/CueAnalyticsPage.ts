...
const transitionArrowMarkup = `<span class="inline-direction-arrow" role="img" aria-label="to">${iconMarkup('arrowRight')}</span>`
...
function bindAnalyticsGraphInteractions(scope: ParentNode) {
  for (const graph of scope.querySelectorAll<HTMLElement>('[data-analytics-graph]')) {
    ...
    const setActivePoint = (point: SVGCircleElement) => {
      ...
      tooltip.innerHTML = `
        <strong>Word ${previousCueNumber} ${transitionArrowMarkup} ${cueNumber}</strong>
