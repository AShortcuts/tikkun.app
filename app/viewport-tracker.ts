...
export class ViewportTracker extends EventEmitter<ViewportTrackerEvents> {
  ...
  private readonly handleScroll = () => this.scheduleUpdate()
  ...
  constructor(book: HTMLElement) {
    ...
    book.addEventListener('scroll', this.handleScroll)
  }
  ...
  destroy() {
    if (this.destroyed) return
    this.destroyed = true
    this.book.removeEventListener('scroll', this.handleScroll)
    if (this.updateFrame) {
      cancelAnimationFrame(this.updateFrame)
      this.updateFrame = 0
    }
  ...
  private scheduleUpdate() {
    if (this.destroyed || this.updateFrame) return

    this.updateFrame = requestAnimationFrame(() => {
      this.updateFrame = 0
      if (this.destroyed || !this.book.isConnected) return
      this.update()
    })
  }
