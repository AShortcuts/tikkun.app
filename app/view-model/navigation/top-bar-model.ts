...
export class TopBarTracker {
  ...
  setLine(
    ...
    if (
      ...
      newInfo.relatedRuns = newInfo.currentRun.leining.runs
        .filter((run) => hasScrollData(run.scroll))
        .map((run) => ({
      ...
      const allInstances = model.generator
        ...
        .filter((leining) => leining.runs.some((run) => hasScrollData(run.scroll)))
...
function createInstanceLink(
  target: LeiningInstance | undefined,
  currentRun: LeiningRun
): Link | null {
  const targetRun = target?.runs.find((run) => hasScrollData(run.scroll))
  if (!target || !targetRun) return null
